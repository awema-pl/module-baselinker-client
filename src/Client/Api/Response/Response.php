...
namespace AwemaPL\BaselinkerClient\Client\Api\Response;
        ...
        if ($this->hasError()) {
            ...
            if ($data['error_code'] === 'ERROR_BAD_PRODUCTS'){
                ...
                throw new BaselinkerApiException(
                     'Baselinker API error.',BaselinkerApiException::ERROR_API_BASELINKER, 409, null, $userMessage, sprintf('[%s]: %s', $data['error_code'], $data['error_message'])
                );
            } else {
                throw new BaselinkerApiException(
                    'Baselinker API error.',BaselinkerApiException::ERROR_API_BASELINKER, 409, null, null, sprintf('[%s]: %s', $data['error_code'], $data['error_message'])
                );
            }
